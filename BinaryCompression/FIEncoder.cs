    ...
    internal class FIEncoder
    {
        ...
        internal FIEncoder(Stream output, FIWriterVocabulary vocabulary)
        {
            ...
            if (vocabulary != null)
                ...
                Vocabulary = new FIWriterVocabulary(vocabulary);
        ...
        internal class FIElement
        {
            ...
            internal FIElement()
            {
                DefaultNamespace = null; // must be null as empty string has meaning
                _nsAttributesTop = -1;
                NamespaceAttributes = new FIAttribute[GROW_SIZE];
                _attributesTop = -1;
                Attributes = new FIAttribute[GROW_SIZE];
            }

            internal string DefaultNamespace { get; set; }

            internal FIAttribute[] Attributes { get; private set; }

            internal int AttributeCount => _attributesTop + 1;

            internal FIAttribute[] NamespaceAttributes { get; private set; }

            internal int NamespaceAttributeCount => _nsAttributesTop + 1;

            internal FIWriterVocabulary.QNameIndex QNameIndex => _qnameIndex;
            ...
            {
                if (_attributesTop == Attributes.Length - 1)
                {
                    var destinationArray = new FIAttribute[Attributes.Length + GROW_SIZE];
                    if (_attributesTop > 0)
                        Array.Copy(Attributes, destinationArray, _attributesTop + 1);

                    Attributes = destinationArray;
                }

                _attributesTop++;
                Attributes[_attributesTop] = attribute;
            }

            internal void AddNamespaceAttribute(FIAttribute nsAttribute)
            {
                if (_nsAttributesTop == NamespaceAttributes.Length - 1)
                {
                    var destinationArray = new FIAttribute[NamespaceAttributes.Length + GROW_SIZE];
                    if (_nsAttributesTop > 0)
                        Array.Copy(NamespaceAttributes, destinationArray, _nsAttributesTop + 1);

                    NamespaceAttributes = destinationArray;
                }

                _nsAttributesTop++;
                NamespaceAttributes[_nsAttributesTop] = nsAttribute;
            }
        }
        ...
        internal FIWriterVocabulary Vocabulary { get; private set; }
        ...
        internal void WriteDeclaraion(FIWriter.FInfoDecl decl)
        {
            ...
            if (decl != FIWriter.FInfoDecl.FInfoDecl_NONE)
                ...
                _output.Write(FIConsts.FI_DECLARATION_LIST[(int)decl - 1], 0,
                    FIConsts.FI_DECLARATION_LIST[(int)decl - 1].Length);
        }
        ...
        internal void WriteHeader()
        {
            ...
            if (Vocabulary == null)
            {
                ...
                Vocabulary = new FIWriterVocabulary();
        ...
        internal void WriteElement(FIElement element)
        {
            ...
            var defaultNamespace = element.DefaultNamespace;
            if (defaultNamespace != null || element.NamespaceAttributeCount != 0)
            {
                ...
                if (defaultNamespace != null)
                {
                    ...
                    _output.WriteByte(0xCD);
                    IdentifyingStringOrIndexBit1(defaultNamespace, Vocabulary.NamespaceNamesMap);
                }

                // add Namespace Attributes
                for (var n = 0; n < element.NamespaceAttributeCount; n++)
                {
                    var attr = element.NamespaceAttributes[n];

                    // must have prefix, so append 11001111
                    _output.WriteByte(0xCF);
                    IdentifyingStringOrIndexBit1(attr.qnameIndex.qname.localName, Vocabulary.PrefixNamesMap);
                    IdentifyingStringOrIndexBit1((string)attr.data, Vocabulary.NamespaceNamesMap);
                }
            ...
            // add Qualified Name
            QualifiedNameOrIndexBit3(byCurrent, element.QNameIndex, Vocabulary.ElementNamesMap);

            // add Attributes
            for (var n = 0; n < element.AttributeCount; n++)
            {
                var attr = element.Attributes[n];

                QualifiedNameOrIndexBit2(0, attr.qnameIndex, Vocabulary.AttributeNamesMap);
                ...
                if (attr.data != null)
                {
                    if (attr.encoding != null)
                        ...
                        NonIdentifyingStringOrIndexBit1((string)attr.data, Vocabulary.AttributeValuesMap);
                    else
        ...
        internal void WriteProcessingInstruction(string target, string content)
        {
            ...
            _output.WriteByte(0xE1);
            IdentifyingStringOrIndexBit1(target, Vocabulary.OtherNCNamesMap);
            NonIdentifyingStringOrIndexBit1(content, Vocabulary.OtherStringMap);
        }
        ...
        internal void WriteComment(string text)
        {
            ...
            _output.WriteByte(0xE2);
            NonIdentifyingStringOrIndexBit1(text, Vocabulary.OtherStringMap);
        }
        ...
        internal void WriteCharacterChunk(string text)
        {
            ...
            // append 10xxxxxx
            NonIdentifyingStringOrIndexBit3(0x80, text, Vocabulary.ContentCharacterChunksMap);
        }
        ...
        private void IdentifyingStringOrIndexBit1(string key, Dictionary<string, int> mapValues)
        {
            ...
            else
            {
                ...
                if (mapValues.Count < FIConsts.TWO_POWER_TWENTY) mapValues.Add(key, mapValues.Count + 1);
            }
        }
        ...
        // C.17
        private void QualifiedNameOrIndexBit2(byte byCurrent, FIWriterVocabulary.QNameIndex qnameIndex,
            FIWriterVocabulary.QNameArray mapQNames)
        {
            ...
            else
            {
                ...
                {
                    var prefixIndex = 0;
                    if (Vocabulary.FindPrefixNameIndex(qnameIndex.qname.prefix, out prefixIndex))
                    {
                        // string-index so add 1xxxxxxx and index
                    ...
                    else
                    {
                        ...
                        Vocabulary.AddPrefixName(qnameIndex.qname.prefix);
                    }
                }
                ...
                if (qnameIndex.qname.ns != null && qnameIndex.qname.ns.Length != 0)
                {
                    ...
                    if (Vocabulary.FindNamespaceNameIndex(qnameIndex.qname.ns, out namespaceIndex))
                    {
                        // string-index so add 1xxxxxxx and index
                    ...
                    else
                    {
                        ...
                        Vocabulary.AddNamespaceName(qnameIndex.qname.ns);
                    }
                }

                var localNameIndex = 0;
                if (Vocabulary.FindLocalNameIndex(qnameIndex.qname.localName, out localNameIndex))
                {
                    // string-index so add 1xxxxxxx and index
                ...
                else
                {
                    ...
                    Vocabulary.AddLocalName(qnameIndex.qname.localName);
                }
            }
        }

        // C.18
        private void QualifiedNameOrIndexBit3(byte byCurrent, FIWriterVocabulary.QNameIndex qnameIndex,
            FIWriterVocabulary.QNameArray mapQNames)
        {
        ...
        {
            switch (Vocabulary.CharacterStringEncoding)
            {
                ...
                default:
                    throw new LtFastInfosetException(
                        "Internal Error in EncodedCharacterStringBit3. Unknown StringEncoding: " +
                        Vocabulary.CharacterStringEncoding);
            }
        }

        private void EncodedCharacterStringBit3(byte byCurrent, FIEncoding encoding, object data)
        {
            var tableIndex = encoding.TableIndex;
            ...
            var byVal = (byte)(tableIndex - 1);

            var buffer = encoding.Encode(data);
            ...
            if (encoding is FIRestrictedAlphabet)
                ...
                throw new LtFastInfosetException(
                    "Internal Error in EncodedCharacterStringBit3. Unknown encoding type.");
        ...
        private void EncodedCharacterStringBit5(byte byCurrent, string val)
        {
            switch (Vocabulary.CharacterStringEncoding)
            {
                ...
                default:
                    throw new LtFastInfosetException(
                        "Internal Error in EncodedCharacterStringBit5. Unknown StringEncoding: " +
                        Vocabulary.CharacterStringEncoding);
            }
        }
        ...
        private int _encodingBufferLength;
