...
namespace LiquidTechnologies.FastInfoset
{
    internal class InternalFIWriter
    {
        ...
        private class NamespaceManager
        {
            ...
            {
                var num = _elementStack[_elementTop].prefixCount++ + 1;
            ...
            {
                for (var i = _namespaceTop; i > _elementStack[_elementTop].prevNamespaceTop; i--)
                    if (_namespaceStack[i].prefix == prefix)
                        return i;
            ...
            internal void PushStack(string prefix, string ns, string localName)
            {
                ...
                {
                    var destinationArray = new ElementInfo[_elementStack.Length * 2];
                    if (_elementTop > 0)
            ...
            internal void PushNamespace(string prefix, string ns)
            {
                ...
                if (prefix != null)
                {
                    ...
                    var index = LookupNamespace(prefix);
            ...
            {
                var length = ++_namespaceTop;
                if (length == _namespaceStack.Length)
                {
                    var destinationArray = new NamespaceInfo[length + NS_GROW_SIZE];
        ...
        internal string LookupPrefix(string ns)
        {
            ...
            var prefix = _namespaceManager.FindPrefix(ns);

            if (prefix == null && ns == _namespaceManager.DefaultNamespace) prefix = string.Empty;
        ...
        internal void WriteStartAttribute(string prefix, string localName, string ns)
        {
            ...
            if (prefix != FIConsts.FI_DEFAULT_PREFIX)
            {
                ...
                else
                {
                    ...
                    var text = _namespaceManager.FindPrefix(ns);
        ...
        private readonly FIEncoder.FIElement _element;
