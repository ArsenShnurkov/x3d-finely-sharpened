...
namespace LiquidTechnologies.FastInfoset
{
    ...
    internal sealed class FIParser
    {
        ...
        internal FIParser(Stream input, FIVocabularyManager vocabularyManager, XmlNameTable nameTable)
        {
            ...
            NameTable.Add(FIConsts.FI_DEFAULT_PREFIX);
            NameTable.Add(FIConsts.FI_XML_NAMESPACE_NAME);
        ...
        private enum EnumStringOrIndex
        {
            isoiString,
            isoiIndex
        }
        ...
        {
            private readonly int _blockSize = 4096; // 4K
            ...
            {
                if (len < 1)
                    throw new LtFastInfosetException("Internal Error in StreamBuffer::ReadBytes. Length = " + len);

                var outBuffer = new byte[len];

                var bytesLeftInBuffer = _bufferSize - _bufferOffset;
            ...
            {
                var offset = 0;
                var count = _blockSize;

                do
                {
                    var bytesRead = _input.Read(_buffer, offset, count);
                    if (bytesRead == 0)
                        break;
        ...
        internal class FINode
        {
            ...
            internal FINode()
            {
                ...
                Depth = 0;
                _attributeTop = -1;
                Attributes = new QNameValue[GROW_SIZE];
            }

            internal QNameValue[] Attributes { get; private set; }

            internal int AttributeCount => _attributeTop + 1;

            internal QualifiedName QName
            {
                get => _nodeValue.qname;
                set => _nodeValue.qname = value;
            }

            internal string Value => _nodeValue.value;

            internal XmlNodeType NodeType { get; private set; } = XmlNodeType.None;
            ...
            {
                if (_attributeTop == Attributes.Length - 1)
                {
                    var destinationArray = new QNameValue[Attributes.Length + GROW_SIZE];
                    if (_attributeTop > 0)
                        Array.Copy(Attributes, destinationArray, _attributeTop + 1);

                    Attributes = destinationArray;
                }

                _attributeTop++;
                Attributes[_attributeTop] = attrNode;
            }

            internal void SetAttributes(QNameValue[] nodeList, int count)
            {
                if (Attributes.Length < count)
                    Attributes = new QNameValue[count];

                Array.Copy(nodeList, Attributes, count);
        ...
        {
            var read = _buffer.ReadBytes(length);
            _lastByte = read[0];
        ...
        private void ReadHeader()
        {
            ...
            if (!CheckHeader())
            {
                ...
                var maxDeclLen = 0;
                foreach (var b in FIConsts.FI_DECLARATION_LIST)
                    if (b.Length > maxDeclLen)
                        maxDeclLen = b.Length;

                // read in enough bytes to cover longest declaration
                var buffer = Move(maxDeclLen);

                // C.12.3
                var found = false;
                foreach (var decl in FIConsts.FI_DECLARATION_LIST)
                    ...
                    if (Utils.CompareByteArrays(buffer, decl, decl.Length))
                    {
                        ...
                        if (CheckHeader())
                        {
                            var strVal = Encoding.UTF8.GetString(decl, 0, decl.Length);
                            _currentNode = new FINode();
                            _currentNode.Init(XmlNodeType.XmlDeclaration, 0, FIConsts.FI_DECLARATION_NAME,
                                strVal.Substring(FIConsts.FI_DECLARATION_START_TAG_CHAR_LEN,
                                    strVal.Length - (FIConsts.FI_DECLARATION_START_TAG_CHAR_LEN +
                                                     FIConsts.FI_DECLARATION_END_TAG_CHAR_LEN)));
                            found = true;
                        }
        ...
        private bool CheckHeader()
        {
            ...
            var header = Move(FIConsts.FI_HEADER_SIZE);
            if (header[0] == 0xE0
                && header[1] == 0
                && header[2] == 0
                && header[3] == 0x01)
        ...
        private void AdditionalData()
        {
            ...
            for (var item = 0; item < LengthOfSequence(); item++)
            {
                // ignore id
...
//				_vocabulary.AddOtherURI(LastNameString);
        ...
        private void ElementNameSurrogates()
        {
            ...
            for (var item = 0; item < LengthOfSequence(); item++) NonIdentifyingStringOrIndexBit1();
        ...
        private void Notations()
        {
            ...
            while (Move() >> 2 == 0x30) NotationBit7();
        }
        ...
        private void UnparsedEntities()
        {
            ...
            while (Move() >> 1 == 0x68) UnparsedEntityBit8();
        }
        ...
        private void ElementBit2()
        {
            ...
            var bHasAttributes = (_lastByte & 0x40) != 0;
            ...
            if ((_lastByte & 0x3F) >> 2 == 0x0E)
            {
                ...
                while (Move() >> 2 == 0x33)
                {
                    ...
                    {
                        var destinationArray = new FINode.QNameValue[_namespaceNodes.Length + NS_GROW_SIZE];
        ...
        private void AtrributeBit2()
        {
            ...
            var qnameValue = new FINode.QNameValue();
        ...
        {
            var bHasSystemIdentifier = (_lastByte & 0x02) != 0;
            var bHasPublicIdentifier = (_lastByte & 0x01) != 0;
        ...
        private void CharacterChunkBit3()
        {
            ...
            {
                _currentNode.Init(_lastType, _depth + 1, string.Empty,
                    Convert.ToString(_vocabulary.CharacterChunk(_lastValue - 1)));
            }
            else
            {
                var chunkVal = LastString;
        ...
        private void Comment()
        {
            if (NonIdentifyingStringOrIndexBit1() == EnumStringOrIndex.isoiIndex)
            {
                _currentNode.Init(XmlNodeType.Comment, _depth + 1, string.Empty,
                    Convert.ToString(_vocabulary.OtherString(_lastValue - 1)));
            }
            else
            {
                var commentVal = LastString;
        ...
        private FINode.QNameValue NamespaceAttributeBit7()
        {
            ...
            var bHasPrefix = (_lastByte & 0x02) != 0;
            var bHasNamespaceName = (_lastByte & 0x01) != 0;
        ...
        private void ReadQName()
        {
            ...
            if (bHasPrefix && !bHasNamespaceName)
                throw new LtFastInfosetException(
                    "Failed to parse FastInfoset. Invalid QName type Prefix without Namespace.");
        ...
        private void EncodedCharacterStringBit3()
        {
            ...
            switch (dwVal >> 4)
            {
                ...
                {
                    throw new LtFastInfosetException(
                        "Failed to parse FastInfoset. Invalid EncodedCharacterString type  on the third bit of an octet.");
                }
            }
        }
        ...
        private void EncodedCharacterStringBit5()
        {
            ...
            switch (dwVal >> 2)
            {
                ...
                {
                    throw new LtFastInfosetException(
                        "Failed to parse FastInfoset. Invalid EncodedCharacterString type  on the fifth bit of an octet.");
                }
            }
        }
        ...
        private void NonEmptyOctetEncodingAlgorithmString(int len, int index)
        {
            ...
            var fiEncoding = _vocabulary.EncodingAlgorithm(index);
        ...
        private int NonEmptyOctetStringBit2Length()
        {
            ...
            {
                throw new LtFastInfosetException(
                    "Failed to parse FastInfoset. Invalid NonEmptyOctetString type  on the second bit of an octet.");
            }
        ...
        private int NonEmptyOctetStringBit5Length()
        {
            ...
            {
                throw new LtFastInfosetException(
                    "Failed to parse FastInfoset. Invalid NonEmptyOctetString type  on the fifth bit of an octet.");
            }
        ...
        private int NonEmptyOctetStringBit7Length()
        {
            ...
            {
                throw new LtFastInfosetException(
                    "Failed to parse FastInfoset. Invalid NonEmptyOctetString type  on the seventh bit of an octet.");
            }
        ...
        private int Integer1To2pw20Bit2()
        {
            ...
            {
                throw new LtFastInfosetException(
                    "Failed to parse FastInfoset. Invalid Encoding of integers 1 To 2^20 type  on the second bit of an octet.");
            }
        ...
        private int Integer0To2pw20Bit2()
        {
            ...
            if (dwVal != 0x7F)
                ...
                {
                    throw new LtFastInfosetException(
                        "Failed to parse FastInfoset. Invalid Encoding of integers 0 To 2^20 type  on the second bit of an octet.");
                }
        ...
        private int Integer1To2pw20Bit3()
        {
            ...
            {
                throw new LtFastInfosetException(
                    "Failed to parse FastInfoset. Invalid Encoding of integers 1 To 2^20 type  on the third bit of an octet.");
            }
        ...
        private int Integer1To2pw20Bit4()
        {
            ...
            {
                throw new LtFastInfosetException(
                    "Failed to parse FastInfoset. Invalid Encoding of integers 1 To 2^20 type  on the fourth bit of an octet.");
            }

            return dwIntVal;
        }

        private string LastString => new string(_lastCharBuffer, 0, _lastCharBufferUsed);

        private string LastNameString => NameTable.Add(_lastCharBuffer, 0, _lastCharBufferUsed);
        ...
        private readonly FIVocabularyManager _vocabularyManager;
        private FIReaderVocabulary _vocabulary;

        private FINode _currentNode;
        private FINode.QNameValue[] _namespaceNodes;
        ...
        private char[] _lastCharBuffer;
        private int _lastCharBufferUsed;
