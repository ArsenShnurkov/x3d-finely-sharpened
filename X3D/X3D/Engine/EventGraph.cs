...
namespace X3D.Engine
{
    ...
    {
        private AutoResetEvent suspend = new AutoResetEvent(false);
...
//            }
        ...
        /// </summary>
        private readonly Queue<EventRoute> queue = new Queue<EventRoute>();

        /// <summary>
        ///     Next events that will be dispatched (carried on) to the next timestamp
        /// </summary>
        private readonly List<EventRoute> nextDispatch = new List<EventRoute>();
        ...
        private readonly Thread propagator;
        ...
        public void PropagateEvents(RenderingContext rc)
        {
            ...
            if (!propagator.IsAlive && !isPropagating) propagator.Start();
        ...
        public void CreateEventGraph()
        {
            ...
            foreach (var route in sg.Routes)
            {
                ...
                if (fHas && tHas) queue.Enqueue(@event);
                if (!fHas) error += " from attribute not found";

                if (!tHas) error += " to attribute not found";

                error = error.Trim();

                if (!string.IsNullOrEmpty(error)) Console.WriteLine("->Error {1} {0}", @event, error);
            }
        }
