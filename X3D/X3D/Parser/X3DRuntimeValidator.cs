...
namespace X3D.Parser
{
    ...
    public class X3DRuntimeValidator
    {
        ...
        /// </summary>
        private static void processInvalidNodes(SceneGraphNode parent, List<SceneGraphNode> invalid, string parentName,
            out bool passed, bool noChildrenAllowed = false)
        {
            string msg;
        ...
        private static void pruneBadRelationships(SceneGraphNode parent, List<SceneGraphNode> invalidNodes)
        {
            ...
            {
                var invalidNode = invalidNodes[i];
            ...
            if (typeof(X3DAppearanceNode).IsInstanceOfType(parent))
            {
                                              ...
                                              || typeof(X3DPrototypeInstance).IsInstanceOfType(n)))
                {
                    ...
                    if (!parent.alreadyWarned && debug)
                        Console.Write("[Warning] {0} doesnt contain any X3DAppearanceChildNode children ", parent);
        ...
        private static void shaderValidationConstraints(SceneGraphNode parent, out bool passed, out bool warned)
        {
            ...
            if (typeof(X3DProgrammableShaderObject).IsInstanceOfType(parent))
            {
                                              ...
                                              || typeof(X3DPrototypeInstance).IsInstanceOfType(n)))
                {
                    ...
                    if (!parent.alreadyWarned && debug)
                        Console.WriteLine("[Warning] {0} doesnt contain any field children", parent);
                }

                if (typeof(ComposedShader).IsInstanceOfType(parent))
                {
                    invalid = parent.Children.Where(n =>
                        !(typeof(ShaderPart).IsInstanceOfType(n) || typeof(field).IsInstanceOfType(n))).ToList();

                    processInvalidNodes(parent, invalid, parent.ToString(), out passed);

                    var shaderParts = parent.Children.Where(n => typeof(ShaderPart).IsInstanceOfType(n))
                        .Select(part => (ShaderPart)part).ToList();
                    ...
                    if (!shaderParts.Any())
                    {
                        ...
                        pruneBadRelationship(parent, true);
                    }
                    else
                    {
                        for (var i = 0; i < shaderParts.Count(); i++)
                        {
                            ...
                            if (part.Children.Any())
                            {
                                ...
                                if (debug)
                                    Console.WriteLine(
                                        "ShaderPart must not have any children defined other than a CDATA text node");
                            ...
                            if (string.IsNullOrEmpty(part.url) && string.IsNullOrEmpty(part.ShaderSource.Trim()))
                            {
                                ...
                                if (debug)
                                    Console.WriteLine("ShaderPart must have a url attribute or CDATA section defined");

                                pruneBadRelationship(part);
                            }
                            else if (!string.IsNullOrEmpty(part.ShaderSource) &&
                                     string.IsNullOrEmpty(part.ShaderSource.Trim()))
                            {
                                passed = false;
                ...
                else if (typeof(PackagedShader).IsInstanceOfType(parent))
                {
                    ...
                    processInvalidNodes(parent, invalid, parent.ToString(), out passed, true);
                }
